...
if __name__=='__main__':
    ...
    if CONFIG['prediction']==False and CONFIG['clerical_review_candidates']==False:
        logger.info('###########')
    ...
    #####IF database_creation mode is true ('create' is legacy), make the DB tables.
    if CONFIG['database_creation_mode'] or CONFIG['database_creation_mode'].lower()=='create':
    ...
    db = get_db_connection(CONFIG)
    batch_summary['batch_id'] = CONFIG['batch_summary_id'] = generate_batch_id(db)
    logger.info('#############')
    logger.info('''Batch ID: {}'''.format(CONFIG['batch_summary_id']))
    ###update the batch summary
    update_batch_summary(batch_summary)
    ###create the database
    try:
        if CONFIG['database_creation_mode']==True:
    ...
    for table_name in ['matched_pairs_table_name','clerical_review_candidates_table_name']:
        if CONFIG.get(table_name, None) is not None:
            if CONFIG['sql_flavor']=='postgres':
                ...
                exists = get_table_noconn('''select name from sqlite_master where type='table' AND name='{}' '''.format(CONFIG[table_name]), db)
                if len(exists) > 0:
                    exists = True
                else:
                    exists = False
    ...
    if CONFIG['prediction']:
        if '.joblib' in CONFIG['saved_model']:
            try:
                ...
                logger.info(
                    'Error in loading model.  Check your .joblib file exists and is set up for your version of sklearn. Error: {}'.format(
                        ''.join(traceback.format_tb(error.__traceback__))))
                batch_summary['batch_status'] = 'failed'
                batch_summary['failure_message'] = 'Failed to select a model.  See logs for information'
                update_batch_summary(batch_summary)
                os._exit(0)
        elif CONFIG['use_custom_model']==True and CONFIG['use_mamba_models']==False:
             ####In this case, we are using a custom model but no mamba models.
             try:
                 mod = load_model(CONFIG, CONFIG['custom_model_name'])
             except Exception as error:
                 logger.info(
                     'Error in loading model.  Check your .joblib file exists and is set up for your version of sklearn. Error: {}'.format(
                         ''.join(traceback.format_tb(error.__traceback__))))
                 ...
                 update_batch_summary(batch_summary)
                 os._exit(0)
        ###Otherwise, we will need training data and to fit it.
        else:
            try:
                if CONFIG['training_data_name'][-4:] != '.csv':
                    training_data_file = os.path.join(CONFIG['projectPath'], CONFIG['training_data_name'], 'csv')
                else:
                    training_data_file = os.path.join(CONFIG['projectPath'],CONFIG['training_data_name'])
                training_data = pd.read_csv(training_data_file, engine='c',
                                            dtype={'{}_id'.format(CONFIG['data1_name']): str,
                                                   '{}_id'.format(CONFIG['data2_name']): str})
            except Exception as error:
                logger.info(
                    'Error in loading training data.  Trying to load {}.  Error: {}'.format(training_data_file,
                        ''.join(traceback.format_tb(error.__traceback__))))
                batch_summary['batch_status'] = 'failed'
                batch_summary['failure_message'] = 'Failed to select a model.  See logs for information'
                update_batch_summary(batch_summary)
            ...
            try:
                ...
                logger.info(
                    'Error in generating model.  Error: {}'.format(''.join(traceback.format_tb(error.__traceback__))))
                batch_summary['batch_status'] = 'failed'
                batch_summary['failure_message'] = 'Failed to select a model.  See logs for information'
                update_batch_summary(batch_summary)
                os._exit(0)
            if CONFIG.get('model_only', None) is not None and CONFIG['model_only']==True:
    ...
    if CONFIG['clerical_review_candidates'] == True and CONFIG['prediction'] == True:
        if 'listed_variable_names' in CONFIG['clerical_review_threshold'].keys():
    ...
    db=get_db_connection(CONFIG)
    pd.DataFrame(get_table_noconn('''select * from {} where batch_summary_id={} '''.format(CONFIG['matched_pairs_table_name'],batch_summary['batch_id']), db)).to_csv('{}/output/all_matches_batch_{}.csv'.format(CONFIG['projectPath'],batch_summary['batch_id']), index=False)
    if CONFIG['clerical_review_candidates']==True:
        pd.DataFrame(get_table_noconn('''select * from {} where batch_summary_id={} '''.format(CONFIG['clerical_review_candidates_table_name'], batch_summary['batch_id']), db)).to_csv('{}/output/clerical_review_candidates_batch_{}.csv'.format(CONFIG['projectPath'],batch_summary['batch_id']), index=False)
    logger.info('Match Complete')
    if CONFIG['prediction']==True:
        summstats=get_table_noconn('''select count(distinct {}_id) data1_matched, count(distinct {}_id) data2_matched, count(*) total_pairs from {} where batch_summary_id={}'''.format(CONFIG['data1_name'], CONFIG['data2_name'], CONFIG['matched_pairs_table_name'], batch_summary['batch_id']), db)[0]
        ...
        (sum(block_matches_avg_score * block_matches)/nullif(sum(block_matches),0)) average_match_score,
         (sum(block_non_matches_avg_score * block_non_matches)/nullif(sum(block_non_matches),0)) average_non_match_score
           from batch_statistics where batch_summary_id = {} group by block_level'''.format(CONFIG['batch_summary_id']),db)
