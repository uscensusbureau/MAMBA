...
create index batch_stat_idx on batch_statistics(batch_summary_id);
...
def get_stem_data(data_source):
    ...
    if CONFIG['mode']!='deduplication':
        ###if we aren't in deduplicaiton then these will all change from item to item
        ...
        ###but if we are in deduplication, these will be the same from item to item, so we can just grab the 'left' from each block/variable type.
        str_dict = {item['left']: str for item in global_vars.blocks}
        fuzzy_vars = [i['left'] for i in global_vars.var_types if i['match_type'].lower() == 'fuzzy']
        date_columns = [i['left'] for i in global_vars.var_types if i['match_type'] == 'date']
    ###now check if there are any variable filters
    for block in global_vars.blocks:
        if 'variable_filter_info' in block and block['variable_filter_info']!={}:
            str_dict[block['variable_filter_info'][dataname]]=str
    ###
    str_dict['id'] = str
    ...
    csvname = os.path.join(CONFIG['projectPath'], dataname)+'.csv'
    ...
    for data in pd.read_csv(csvname, chunksize=int(CONFIG['create_db_chunksize']), engine='c', dtype=str_dict):
        ...
        data['matched'] = 0
        ###quick check if any zipcode strings are the right length
        for string_var in str_dict.keys():
            if 'zip' in string_var.lower():
                max_len = max([len(x) for x in data[string_var].to_list()])
                logger.info('Variable {} appears to be a zipcode variable.  I will zero-pad it to {} characters long'.format(string_var, max_len))
                data[string_var] = data[string_var].str.zfill(max_len)
        ...
        if any([k['block_name']=='full' for k in blocks]):
            row_seq = [np.floor(i/int(10000)) for i in list(range(row_number, row_number+len(data)))]
            data['full'] = row_seq
            row_number += len(data)
        ...
        ########
        '''
        Deprecated for v1.1, there are better ways to do this.
        if ast.literal_eval(CONFIG['parse_address']) == True:
        ...
        if table_exists==False:
            ...
            if outcome =='success':
                ...
                logger.info('Trying to overwrite an existing table.  Breaking.')
                break
        # 4) Push to DB
        if CONFIG['prediction'] == True:
            ...
            if write_out:
                ...
                break
        if CONFIG['prediction'] == False and CONFIG['clerical_review_candidates'] == True:
            data = pd.DataFrame(data).sample(frac=.05).to_dict('records')
            write_out= write_to_db(data, data_source)
            if write_out:
                logger.info('Failed to write data to database, breaking')
                break
...
def createDatabase(databaseName):
    ...
    if CONFIG['mode']=='deduplication':
        data_sources = [CONFIG['target_table'], '{}_training'.format(CONFIG['target_table'])]
    else:
        data_sources = [CONFIG['data1_name'], CONFIG['data2_name'], '{}_training'.format(CONFIG['data1_name']),
         '{}_training'.format(CONFIG['data2_name'])]
        ...
        try:
            if os.path.isfile('{}/{}.csv'.format(CONFIG['projectPath'],data_source))==True:
                ...
                for i in global_vars.blocks:
                    cur.execute('''create index if not exists {source}_{variable}_idx on {source} ({variable});'''.format(variable=i[data_source.split('_training')[0]], source=data_source))
    ...
    if CONFIG['sql_flavor']=='postgres':
        if CONFIG['prediction']:
            cur.execute('''create sequence if not exists matched_pairs_seq;create sequence if not exists clerical_review_candidates_seq''')
            db.commit()
            cur.execute('''create table {matched_pair_table_name} (
            id bigint default nextval(matched_pairs_seq), 
            {data1}_id bigint, 
            {data2}_id bigint,
             predicted_probability float, 
             {data1}_rank float, 
             {data2}_rank float, 
             batch_summary_id float, 
             match_pair_info json);'''.format(matched_pair_table_name=CONFIG['matched_pairs_table_name'],
                                              data1=CONFIG['data1_name'], data2=CONFIG['data2_name']))
            db.commit()
        if CONFIG['clerical_review_candidates']:
            cur.execute('''create table {clerical_review_candidates_table_name} (
            id bigint default nextval(clerical_review_candidates_seq), 
            {data1}_id bigint, 
            {data2}_id bigint,
             predicted_probability float, 
             threshold_values json,
             threshold_value float,
             batch_summary_id float );'''.format(clerical_review_candidates_table_name=CONFIG['clerical_review_candidates_table_name'],
                                              data1=CONFIG['data1_name'], data2=CONFIG['data2_name']))
            db.commit()
    else:
        if CONFIG['prediction']:
            ...
            id integer primary key autoincrement, 
                    ...
                    {data2}_id bigint,
                     ...
                     {data2}_rank float, 
                     batch_summary_id float, 
                     match_pair_info json);'''.format(matched_pair_table_name=CONFIG['matched_pairs_table_name'], data1=CONFIG['data1_name'], data2=CONFIG['data2_name']))
            db.commit()
        if CONFIG['clerical_review_candidates']:
            cur.execute('''create table {clerical_review_candidates_table_name} (
                    id integer primary key autoincrement, 
                    ...
                    {data2}_id bigint,
                     ...
                     threshold_values json,
                     threshold_value float,
                     batch_summary_id float );'''.format(
                clerical_review_candidates_table_name=CONFIG['clerical_review_candidates_table_name'],
                data1=CONFIG['data1_name'], data2=CONFIG['data2_name']))
            db.commit()
