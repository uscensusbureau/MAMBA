...
def generate_logit(y, X, X_hdrs):
    ...
    X=X-X.mean()
    if CONFIG['feature_elimination_mode'] == False:
        myparams = {
            'solver':['newton-cg', 'lbfgs', 'liblinear', 'sag', 'saga']}
        mod=LogisticRegression(random_state=0, solver='liblinear')
        cv_rfc = RandomizedSearchCV(estimator=mod, param_distributions=myparams, cv=10, scoring=CONFIG['scoringcriteria'],n_iter=niter)
        cv_rfc.fit(X, y)
        ...
        cv_rfc = RandomizedSearchCV(estimator=selector, param_distributions=myparams, cv=10, scoring=CONFIG['scoringcriteria'],
                                    n_iter=niter)
        cv_rfc.fit(X, y)
        preds = cv_rfc.predict(X)
        score = cv_rfc.score(X, y)
...
def generate_rf_mod(y, X, X_hdrs):
    ...
    if CONFIG['debugmode']==True:
        ...
        niter = 50
    if CONFIG['feature_elimination_mode'] == False:
        cv_rfc = RandomizedSearchCV(estimator=rf, param_distributions=myparams, cv=5, scoring=CONFIG['scoringcriteria'],n_iter=niter)
        cv_rfc.fit(X, y)
        ##Save the parameters
        ...
        selector = RFECV(rf, step=1, cv=25, scoring=CONFIG['scoringcriteria'])
        selector.fit(X,y)
        cv_rfc = RandomizedSearchCV(estimator=selector, param_distributions=myparams, cv=10, scoring=CONFIG['scoringcriteria'],
                                    n_iter=niter)
        cv_rfc.fit(X, y)
...
def generate_ada_boost(y, X, X_hdrs):
    ...
    if CONFIG['feature_elimination_mode']==False:
        ##First, get the scores
        cv_rfc = RandomizedSearchCV(estimator=ada, param_distributions=myparams, cv=10, scoring=CONFIG['scoringcriteria'],
                                    n_iter=niter)
        cv_rfc.fit(X, y)
        ##Save the parameters
        trees = cv_rfc.best_params_['n_estimators']
        ...
        selector = RFECV(ada, step=1, cv=25, scoring=CONFIG['scoringcriteria'])
        selector.fit(X,y)
        cv_rfc = RandomizedSearchCV(estimator=selector, param_distributions=myparams, cv=10, scoring=CONFIG['scoringcriteria'],
                                    n_iter=niter)
        cv_rfc.fit(X, y)
...
def generate_svn_mod(y, X, X_Hdrs):
    ...
    if CONFIG['feature_elimination_mode']==False:
        ##First, get the scores
        svn_rfc = RandomizedSearchCV(estimator=svc, param_distributions=myparams, cv=5, scoring=CONFIG['scoringcriteria'])
        svn_rfc.fit(X, y)
        ##Save the parameters
